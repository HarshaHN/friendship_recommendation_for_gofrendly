...
#%%-----------------------------------
""" 01. Data for the test evaluation """
import pickle 

trainmf = pd.read_hdf("../data/common/train/all_network.h5", key='mf') #120,409
testmf = pd.read_hdf("../data/common/test/all_network.h5", key='mf') #148,904
trainmf = set(tuple(zip(trainmf.user_id, trainmf.friend_id)))
testmf = set(tuple(zip(testmf.user_id, testmf.friend_id)))

def sub(links, subids):
    return set([(a,b) for (a,b) in links if ((a in subids) and (b in subids))])

ids = list(pd.read_hdf("../data/common/train/sqlusers.h5", key='01').user_id)

# trianpos, valpos = 16063 (2134), 904
testpos = list(sub(testmf - trainmf, ids)) #2596

id_idx = {id: n for n,id in enumerate(ids)} # dict(enumerate(feat.index))
testpos2 = [(id_idx[a], id_idx[b]) for a,b in testpos]

#with open('../data/common/test/testpos.pkl', 'wb') as f: pickle.dump(testpos2, f) #2596
#with open('../data/common/test/testpos_ri.pkl', 'wb') as f: pickle.dump(testpos, f) #2596

# del f, trainmf, testmf, trainmf2, testmf2, trainpos, testpos, ids 

#%%


#%%-----------------------------------
""" 00. To get indices for Exp. 02. """
import pickle
import pandas as pd

sqlusers = pd.read_hdf("../data/common/train/sqlusers.h5", key='01')
# ['user_id', 'story', 'iam', 'meetfor', 'birthday', 'marital', 'children', 'lat', 'lng']
x1_idx = list(sqlusers.user_id)

all_sqlusers = pd.read_hdf("../data/common/train/all_sqlusers.h5", key='01')
#['id', 'story', 'iam', 'meetfor', 'marital', 'kids', 'age', 'lat', 'lng']
all_sqlusers['user_id'] = all_sqlusers.index
all_sqlusers.reset_index(drop=True, inplace=True)
# ['id', 'story', 'iam', 'meetfor', marital', 'kids', 'age', 'lat', 'lng','user_id']
x3_idx = list(all_sqlusers.user_id)

idx = []
for i in x1_idx:
  idx.append(all_sqlusers.index[all_sqlusers['user_id'] == i].tolist()[0])

#with open('../data/tensors/s2_idx.pkl', 'wb') as f: pickle.dump(torch.tensor(idx), f)

#%%-

print(x1_idx == [x3_idx[i] for i in idx])
check = list(pd.read_hdf('../data/one/all_processed_users.h5', key='01').index)
print(check == list(all_sqlusers.user_id))
