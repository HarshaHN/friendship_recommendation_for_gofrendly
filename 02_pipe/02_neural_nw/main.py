...
# user_features = user_features[:, 3:48]
# user_features = user_features[:, 48:]
# with open('data/one.pkl', 'rb') as f: user_features = pickle.load(f)
# neg = neg; pos = pos  #402761: (2134,400627) #72382: (16063,56319)
 
fdim = user_features.shape[1]
#idx = list(range(user_features.shape[0])); trainpos = pos
with open('data/rwG_s2.pkl', 'rb') as f: rwG = pickle.load(f) # 121109: 180963, 404803
 
twomodel = nn.gnet( graph = rwG,
                    nodeemb = user_features, #.to(device),
...
onepipe = pipe.pipeflow(user_features[idx], K=500)
...
import torch
user_features[2]/torch.norm(X[2])
